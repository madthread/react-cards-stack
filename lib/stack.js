...
;(function(window) {
	...
	Stack.prototype._init = function() {
		...
		for(var i = 0; i < this.itemsTotal; ++i) {
			...
			if( i < this.options.visible ) {
				...
				item.style.WebkitTransform = item.style.transform = 'translate3d(' + parseInt(1 * 7 * i) + 'px' + ', 0px, ' + parseInt(1 * 14 * i) + 'px)';
			}
			else {
				item.style.WebkitTransform = item.style.transform = 'translate3d(' + parseInt(this.options.visible * 7) + 'px' + ',0,' + parseInt(this.options.visible * 14) + 'px)';
			}
		}
	...
	Stack.prototype._next = function(action, callback) {
		...
		onEndAnimation(currentItem, function() {
			...
			currentItem.style.WebkitTransform = currentItem.style.transform = 'translate3d(0px, 0px, ' + parseInt(self.options.visible * 14) + 'px)';
			...
			var item = this.items[pos],
				...
				animateStackItems = function(item, i) {
					...
					dynamics.animate(item, {
						translateX : parseInt(1 * 7 * i),
						translateZ : parseInt(1 * 14 * i)
			...
			setTimeout(function(item,i) {
				return function() {
					...
					if( preAnimation ) {
						...
						animProps.translateZ = parseInt(-1 * 40 * (i+1));
